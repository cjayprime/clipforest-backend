...
class AnalysisHandler(JobHandler):
    ...
    async def run(self, ctx: JobContext) -> dict | None:
        ...
        # Re-analysing a video that was already READY is free. Otherwise this is a
        # no-op unless a retry resumed here after the earlier charge was refunded.
        if v["ready_at"] is None:
        ...
        await db.replace_candidates(vid, str(row["transcript_id"]), run_no, s.analysis_version, f"{result.provider}:{result.model}", rows)
