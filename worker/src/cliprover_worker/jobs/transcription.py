...
from .base import JobContext, JobHandler, heavy_cpu
...
class TranscriptionHandler(JobHandler):
    ...
    async def run(self, ctx: JobContext) -> dict | None:
        ...
        # Normally already charged at ingest, making this a no-op. It matters when a
        # retry resumes here after the earlier failure was refunded.
        ...
        if row["status"] != "COMPLETED":
            ...
            gate = heavy_cpu if provider.heavy_cpu else contextlib.nullcontext()
            async with gate:
