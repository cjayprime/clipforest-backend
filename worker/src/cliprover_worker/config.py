...
class Settings:
    database_url: str = field(default_factory=lambda: _str("DATABASE_URL", "postgresql://cliprover:cliprover@localhost:5432/cliprover"))
    redis_url: str = field(default_factory=lambda: _str("REDIS_URL", "redis://localhost:6379"))
    queue_prefix: str = field(default_factory=lambda: _str("QUEUE_PREFIX", "cliprover"))
    events_channel: str = field(default_factory=lambda: _str("EVENTS_CHANNEL", "cliprover:events"))
    ...
    s3_bucket: str = field(default_factory=lambda: _str("S3_BUCKET", "cliprover"))
    ...
    # Credits. The API refuses work up front and the worker debits it, so both
    # must read the same values (Compose passes them to both from one place).
    credits_enforced: bool = field(default_factory=lambda: _bool("CREDITS_ENFORCED", False))
    credit_cost_per_source_minute: int = field(default_factory=lambda: _int("CREDIT_COST_PER_SOURCE_MINUTE", 1))
    credit_cost_per_render: int = field(default_factory=lambda: _int("CREDIT_COST_PER_RENDER", 1))
