"""Pipeline error type and taxonomy (PRD §19.3): stable codes, user-facing messages and retry semantics (PRD §14.4)."""
...
class PipelineError(Exception):
    ...
    `retryable` is what the user sees: whether they may retry the video or clip.
    `auto_retry` is whether the queue should retry on its own, and defaults to
    `retryable`. They differ for failures the user has to fix first — running
    out of credits is worth retrying once they top up, but retrying it
    automatically a few seconds later would only fail again.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.auto_retry = retryable if auto_retry is None else auto_retry
        self.details = details or {}

    def __repr__(self) -> str:  # pragma: no cover
        return f"PipelineError({self.code!r}, retryable={self.retryable}, auto_retry={self.auto_retry})"
...
def insufficient_credits(needed: int, available: int) -> PipelineError:
    # Retryable by the user once they top up; never retried automatically, since
    # the balance will not have changed a few seconds later.
    return PipelineError(
        "BILLING_INSUFFICIENT_CREDITS",
        f"This needs {needed} credits and you have {available}. Top up, then retry.",
        retryable=True,
        auto_retry=False,
        details={"needed": needed, "available": available},
    )
