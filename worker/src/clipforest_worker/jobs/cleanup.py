...
class CleanupHandler(JobHandler):
    ...
    async def janitor(self, ctx: JobContext) -> dict:
        ...
        for row in await db.stale_queued_videos():
            await queues.ingest(str(row["id"]), row["pipeline_version"], int(row["processing_run"]), None)
            stats["requeuedVideos"] += 1
        for row in await db.stale_queued_renders():
            await queues.render(str(row["id"]), str(row["video_id"]), int(row["attempt"]), None)
            stats["requeuedRenders"] += 1

        for row in await db.unpurged_deleted_videos():
            await queues.cleanup("purge-video", str(row["id"]), {"videoId": str(row["id"]), "userId": str(row["user_id"])})
            stats["purges"] += 1

        for row in await db.deleted_render_outputs():
            prefix = keys.render_prefix(str(row["user_id"]), str(row["video_id"]), str(row["id"]))
            await storage.delete_prefix(prefix)
            await db.update_render(str(row["id"]), output_key=None, thumbnail_key=None)
            stats["outputsDeleted"] += 1

        if s.render_retention_days > 0:
            for row in await db.expired_render_outputs(s.render_retention_days):
                await storage.delete_prefix(keys.render_prefix(str(row["user_id"]), str(row["video_id"]), str(row["id"])))
                await db.update_render(str(row["id"]), output_key=None, thumbnail_key=None)
                stats["outputsDeleted"] += 1
        ...
        if s.source_retention_days > 0:
            for row in await db.expired_sources(s.source_retention_days):
                ...
                await db.update_video(str(row["id"]), source_expired_at=datetime.now(timezone.utc))
